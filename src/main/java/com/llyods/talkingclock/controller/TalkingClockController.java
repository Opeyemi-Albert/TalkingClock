...
public class TalkingClockController {
  ...
  @Operation(
      description = "Get endpoint for human-friendly time",
      summary = "This is an endpoint " + "to get human-friendly time from the digital values",
      responses = {
        @ApiResponse(description = "Success", responseCode = "200"),
        @ApiResponse(description = "Bad Request", responseCode = "400")
      })
  @GetMapping(value = ApiRoute.DIGITAL_VALUE)
  public ResponseEntity<ResponseHandler> getTalkingClockValue(@PathVariable String digits) {
    return ResponseEntity.ok(
        ResponseHandler.builder()
            .status(HttpStatus.OK)
            .data(talkingClockService.getTalkingClockValue(digits))
            .message("Friendly Time displayed")
            .build());
  }
}
