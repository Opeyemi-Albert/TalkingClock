...
public class TalkingClockServiceImpl implements TalkingClockService {
  ...
  public String convertToHumanFriendlyText(String time) {
    ...
    extractedHour = (extractedHour > ROTATION) ? extractedHour - ROTATION : extractedHour;
    ...
    if (minute == 0) {
      ...
      String clockDirection =
          (minute <= HALF_PAST_MINUTE)
              ? String.valueOf(Direction.past)
              : String.valueOf(Direction.to);
      return String.format(
          "%s %s %s ",
          MINUTE_WORDS[minute / MINUTE_SEQUENCE],
          clockDirection,
          HOUR_WORDS[extractedHour + (minute <= 30 ? 0 : 1)]);
    }
  }
